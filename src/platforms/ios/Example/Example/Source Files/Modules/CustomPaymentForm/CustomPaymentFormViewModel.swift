//
//  CustomPaymentFormViewModel.swift
//  Example
//

import Foundation

final class CustomPaymentFormViewModel {
    // MARK: Properties
    ...
    /// Performs ZIP transaction
    func performZIPTransaction() {
        paymentTransactionManager.performAPMTransaction(jwt: nil, apm: .zip, transactionResponseClosure: handleResponseClosure)
    }
    
    /// Performs ATA transaction
    func performATATransaction() {
        paymentTransactionManager.performAPMTransaction(jwt: nil, apm: .ata, transactionResponseClosure: handleResponseClosure)
    }
    ...
    @discardableResult
    func validateForm(view: CustomPaymentFormView) -> Bool {
